import ImageConstants from "../Components/ImageConstants/ImagesConstants";
export const ArticleDetails = [
  {
    image: ImageConstants.Product13,
    title: " Inside a playful, bohemian beach house on Martha's Vineyard",
    description: "",
  },
  {
    image: ImageConstants.Product14,
    title: " Modern Texas home is beautiful and completely kid-friendly",
   
  },
  {
    image: ImageConstants.Product15,
    title: "  MTudor-style home in Chicago's North Shore is chock-full",
   
  },
];
